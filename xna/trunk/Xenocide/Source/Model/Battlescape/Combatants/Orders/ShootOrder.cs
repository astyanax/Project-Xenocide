...
namespace ProjectXenocide.Model.Battlescape.Combatants
{
    ...
    public partial class ShootOrder : Order
    {
        ...
        private void Fire()
        {
            ...
            shotHits = Xenocide.Rng.RollDice(actionInfo.ChanceToHit(Combatant, activeArm));
            if (shotHits)
            {
        ...
        private Vector3 MissedShotLocation()
        {
            ...
            */
            int radius = Math.Min(3, (int)(Math.Abs(target.X - Combatant.Position.X) + Math.Abs(target.Z - Combatant.Position.Z)));
            List<Vector3> locations = new List<Vector3>();
            for (int dx = -radius; dx <= radius; ++dx)
            {
                for (int dz = -radius; dz <= radius; ++dz)
                {
                    if ((0 != dx || 0 != dz) && Math.Abs(dx) + Math.Abs(dz) <= radius)
                    {
                        // Visibily code is based on Terrain.UpdateVisibility, modified to fit the simpler situation at hand
                        Vector3 testLocation = new Vector3(target.X + dx, 0, target.Z + dz);
                        Vector3 diff = testLocation - Combatant.Position;
                        diff.Normalize();

                        if (Battlescape.Terrain.IsOnTerrain(testLocation) &&
                            Combatant.DotFieldOfView < Vector3.Dot(diff, Combatant.HeadingVector))
                        {
            ...
            if (locations.Count == 0) return target;
            int index = Xenocide.Rng.Next(locations.Count);
            Vector3 selectedLocation = locations[index];
            Vector3 endPos = new Vector3();
            if (!Battlescape.Terrain.IsLineOfSight(Combatant.Position, selectedLocation, VisibilityChecks.LineOfFire, ref endPos))
            {
                selectedLocation.X = (int)endPos.X;
                selectedLocation.Y = (int)endPos.Y;
                selectedLocation.Z = (int)endPos.Z;
            }
        ...
        private Vector3 CalcTrajectoryEnd()
        {
            ...
            if (0 != Battlescape.Terrain.FindCombatantAt(endPoint))
            {
                endPoint.Y += 0.3f;
            }
            return endPoint;
        ...
        private void DamageCombatant(Combatant victim)
        {
            ...
            if (!victim.IsDead)
            {
                ...
                // victim died and is from an opposing team?
                if (victim.IsDead &&
                    ((Combatant.TeamId == Team.XCorp && victim.TeamId == Team.Aliens) || Combatant.TeamId != victim.TeamId))
                {
        ...
        /// <summary>Arm holding item used for the order.</summary>
        private Combatant.ActiveArm activeArm;
        #endregion
    }
}
