...
namespace CeGui
{
    public struct Point { public float X; public float Y; public Point(float x, float y) { X = x; Y = y; } public static implicit operator Point(PointF p) { return new Point(p.X, p.Y); } public static implicit operator PointF(Point p) { return new PointF(p.X, p.Y); } public static implicit operator UiPoint(Point p) { return new UiPoint(p.X, p.Y); } public static implicit operator Point(UiPoint p) { return new Point(p.X, p.Y); } }
    public struct Size { public float Width; public float Height; public Size(float w, float h) { Width = w; Height = h; } public static implicit operator Size(SizeF s) { return new Size(s.Width, s.Height); } public static implicit operator SizeF(Size s) { return new SizeF(s.Width, s.Height); } public static implicit operator UiSize(Size s) { return new UiSize(s.Width, s.Height); } public static implicit operator Size(UiSize s) { return new Size(s.Width, s.Height); } }
    public struct Rect { public float Left; public float Top; public float Right; public float Bottom; public float Width { get { return Right - Left; } } public float Height { get { return Bottom - Top; } } public Rect(float left, float top, float right, float bottom) { Left = left; Top = top; Right = right; Bottom = bottom; } public static implicit operator UiRect(Rect r) { return new UiRect(r.Left, r.Top, r.Right, r.Bottom); } public static implicit operator Rect(UiRect r) { return new Rect(r.Left, r.Top, r.Right, r.Bottom); } }
    ...
    public class WindowSheet : GuiSheet { public Point MousePosition { get; set; } public GuiManager GuiManager { get; set; } }
    ...
    public class GuiSystem { private static GuiSystem _guiSystem; public static GuiSystem Instance => _guiSystem ?? (_guiSystem = new GuiSystem()); public GuiSheet GuiSheet { get; set; } public CeGui.Renderers.Renderer Renderer { get; set; } = new CeGui.Renderers.Renderer(); public void SetDefaultFont(string name) { } public void SetDefaultMouseCursor(Image image) { } }
    public class ImagesetManager { private static ImagesetManager _imagesetManager; public static ImagesetManager Instance => _imagesetManager ?? (_imagesetManager = new ImagesetManager()); public Imageset GetImageset(string name) { return new Imageset(); } public Imageset CreateImageset(string name) { return new Imageset(); } public Imageset CreateImageset(string name, string filename) { return new Imageset(); } public Imageset CreateImageset(string name, Texture texture) { return new Imageset(); } public bool IsImagesetPresent(string name) { return false; } }
    public class WindowManager { private static WindowManager _windowManager; public static WindowManager Instance => _windowManager ?? (_windowManager = new WindowManager()); public Window CreateWindow(string type, string name) { return new GuiSheet(); } public Window GetWindow(string name) { return new WindowSheet(); } public void DestroyWindow(string name) { } public void DestroyWindow(Window window) { } public Window LoadWindowLayout(string filename) { return null; } public Window LoadWindowLayout(string filename, string prefix) { return null; } public Window LoadWindowLayout(string filename, Window prefix) { return null; } public Window LoadWindowLayout(string filename, object prefix) { return null; } }
    ...
    public class AnimationManager { private static AnimationManager _animationManager; public static AnimationManager Instance => _animationManager ?? (_animationManager = new AnimationManager()); public Animation GetAnimation(string name) { return null; } public void LoadAnimation(string name, string filename) { } }
    ...
    public class GuiBuilder { public static GuiSheet BuildGuiSheet(string name) { return new GuiSheet(); } public static Widgets.StaticText CreateText(string text) { return new Widgets.StaticText { Name = text }; } public static Widgets.PushButton CreateButton(string id) { return new Widgets.PushButton { Name = id }; } public static Widgets.Checkbox CreateCheckbox(string id) { return new Widgets.Checkbox { Name = id }; } public static Widgets.ComboBox CreateComboBox(string id) { return new Widgets.ComboBox { Name = id }; } public static Widgets.EditBox CreateEditBox(string id) { return new Widgets.EditBox { Name = id }; } public static Widgets.FrameWindow CreateFrameWindow(string id) { return new Widgets.FrameWindow { Name = id }; } public static Widgets.MultiColumnList CreateGrid(string id) { return new Widgets.MultiColumnList { Name = id }; } public static Widgets.StaticImage CreateImage(string id) { return new Widgets.StaticImage { Name = id }; } public static Widgets.Listbox CreateListBox(string id) { return new Widgets.Listbox { Name = id }; } public static Widgets.Slider CreateSlider(string id) { return new Widgets.Slider { Name = id }; } }
...
{
    public class Renderer { private static Renderer _renderer; public static Renderer Instance => _renderer ?? (_renderer = new Renderer()); public float Width { get; set; } public float Height { get; set; } public void Render() { } public global::CeGui.Texture CreateTexture() { return new global::CeGui.Texture(); } public global::CeGui.Texture CreateTexture(string name, string filename, string area) { return new global::CeGui.Texture(); } }
}

namespace CeGui.Renderers.Xna
{
    public class XnaRenderer { private static XnaRenderer _xnaRenderer; public static XnaRenderer Instance => _xnaRenderer ?? (_xnaRenderer = new XnaRenderer()); public void Render() { } }
}
