...
///   a <see cref="WindowFactory"/> object which is registered with the
///   <see cref="WindowFactoryManager"/>. Additionally, the WindowManager tracks every
...
public class WindowManager : IDisposable {
  ...
  ///   A valid <see cref="WindowFactory"/> for the specified type must be registered.</param>
  ...
  ///   No <see cref="WindowFactory"/> is registered for <paramref name="type"/> <see cref="Window"/> objects.
  /// </exception>
  ...
  Window LoadWindow(XmlNode windowNode, object guiHandler, Hashtable eventMap) {
    ...
    if(null != window) {
      // read the properties
      foreach(XmlNode property in windowNode.SelectNodes("Property")) {
      ...
      if(null != guiHandler) {
        ...
        foreach(XmlNode evt in windowNode.SelectNodes("Event")) {
      ...
      foreach(XmlNode child in windowNode.SelectNodes("Window")) {
        Window temp = LoadWindow(child, guiHandler, eventMap);
  ...
  public Window LoadWindowLayout(string fileName, object guiHandler) {
    ...
    XmlNode rootNode = doc.SelectSingleNode("//GUILayout");
    ...
    if(null != guiHandler) {
      ...
      while(handlerType != typeof(object)) {
    ...
    // load the windows
    foreach(XmlNode windowNode in rootNode.SelectNodes("Window")) {
      Window window = LoadWindow(windowNode, guiHandler, eventMap);
