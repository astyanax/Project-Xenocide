...
///		Single line editbox.
/// </summary>
[ AlternateWidgetName("TaharezLook.TLEditBox") ]
public class TLEditBox : EditBox {
  ...
  public TLEditBox(string type, string name)
    : base(type, name) {
  }
  ...
  /// <returns></returns>
  protected override int GetTextIndexFromPosition(PointF point) {
    //
  ...
  protected override void DrawSelf(float z) {
    ...
    Colour color = new Colour(1, 1, 1, EffectiveAlpha);
    ...
    Colour tmp = normalTextColor;
    tmp.Alpha = EffectiveAlpha;
    colors = new ColourRect(tmp, tmp, tmp, tmp);
