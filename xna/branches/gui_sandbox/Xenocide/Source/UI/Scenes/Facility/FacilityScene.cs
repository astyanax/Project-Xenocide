...
namespace Xenocide.UI.Scenes.Facility
{
    ...
    public class FacilityScene : IDisposable
    {
        ...
        public void LoadGraphicsContent(ContentManager content, GraphicsDevice device, bool loadAllContent)
        {
            if (loadAllContent)
            {
                InitializeEffect(device);
                grid.LoadGraphicsContent(device);
                models.LoadGraphicsContent(content);
                buildTimes.LoadGraphicsContent(content, device);
            }
        }
