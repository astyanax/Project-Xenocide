...
[ AlternateWidgetName("TaharezLook.TLButton") ]
public class TLButton : PushButton {
  ...
  public TLButton(string type, string name)
    : base(type, name) {
  ...
  protected override void DrawDisabled(float z) {
    ...
    Colour colorVal = new Colour(0.7f, 0.7f, 0.7f, EffectiveAlpha);
  ...
  protected override void DrawNormal(float z) {
    ...
    // render clients custom image if that is required.
    if(useNormalImage) {
      ColourRect imageColors = normalImage.Colors;
      imageColors.SetAlpha(EffectiveAlpha);
      Vector3 imagePos = new Vector3(
        absRect.Left, absRect.Top, GuiSystem.Instance.Renderer.GetZLayer(1)
      );
  ...
  protected override void DrawHover(float z) {
    ...
    // render clients custom image if that is required.
    if(useHoverImage) {
      ColourRect imageColors = hoverImage.Colors;
      imageColors.SetAlpha(EffectiveAlpha);
      Vector3 imagePos = new Vector3(
        absRect.Left, absRect.Top, GuiSystem.Instance.Renderer.GetZLayer(1)
      );
  ...
  protected override void DrawPushed(float z) {
    ...
    // render clients custom image if that is required.
    if(usePushedImage) {
      ColourRect imageColors = pushedImage.Colors;
      imageColors.SetAlpha(EffectiveAlpha);
      Vector3 imagePos = new Vector3(absRect.Left, absRect.Top, GuiSystem.Instance.Renderer.GetZLayer(1));
